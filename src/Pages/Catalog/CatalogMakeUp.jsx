...
const CatalogMakeUP = () =>
{
    const {cosmetics, getCosmetics} = useContext(CustomContext);
    ...
    return(
        ...
        <div className="linkFromCat">
             <Link className='link'to='/'>Главная</Link><span> - Косметика</span>
        </div>
        <div className="rightCatalogTtems">
            ...
            <div className="list">
                {
                    cosmetics.sort((a,b) => sort =='more' ? b.price - a.price : sort == 'less'? a.price - b.price : '').filter(el => el.group.includes(groupFilter?.group)).filter(el => el.name.toLowerCase().includes(groupFilter?.name?.toLowerCase())).map((item) => (
