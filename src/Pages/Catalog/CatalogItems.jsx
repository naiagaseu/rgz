...
const CatalogItems = () =>
{
    const {user, other, getOthers} = useContext(CustomContext);
    ...
    return(
        <div>
            <div className="linkFromCat">
                 <Link className='link'to='/'>Главная</Link><span> - Прочее</span>
            </div>
            <div className="rightCatalogTtems">
                ...
                <div className="list">
                    {
                        other.sort((a,b) => sort =='more' ? b.price - a.price : sort == 'less'? a.price - b.price : '').filter(el => el.group.includes(groupFilter?.group)).filter(el => el.name.toLowerCase().includes(groupFilter?.name?.toLowerCase())).map((item) => (
