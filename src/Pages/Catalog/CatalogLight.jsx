...
const CatalogLight = () =>
{
    const {user, lights, getLights} = useContext(CustomContext);
    ...
    return(
        <div>
            <div className="linkFromCat">
                 <Link className='link'to='/'>Главная</Link><span> - Лайтстики</span>
            </div>
            ...
            <div className="search">
                ...
                <div className="list">
                    {
                        lights.sort((a,b) => sort =='more' ? b.price - a.price : sort == 'less'? a.price - b.price : '').filter(el => el.group.includes(groupFilter?.group)).filter(el => el.name.toLowerCase().includes(groupFilter?.name?.toLowerCase())).map((item) => (
